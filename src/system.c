...
void controlByManager(const char* User_Account_File_Name, const char* User_Info_File_Name, int select, pUserAccountStruct* ppUser_Account, pStudentInfoStruct* ppUser_Info) {
	...
	case 1:
		...
		break;
	case 2:
		addStudentInfo(User_Info_File_Name, ppUser_Info);
		break;
	case 3:
		updateStudentInfo(User_Info_File_Name, ppUser_Info);
		break;
	case 4:
		deleteStudentInfo(User_Info_File_Name, ppUser_Info);
		break;
	case 5:
		//printf("------------:查询成功，最新账户信息为:\n");
		...
		break;
	case 6:
		addUserAccount(User_Account_File_Name, ppUser_Account);
		break;
	case 7:
		updateUserAccount(User_Account_File_Name, ppUser_Account);
		break;
	case 8:
		deleteUserAccount(User_Account_File_Name, ppUser_Account);
		break;
	default:
		break;
	}
...
void controlByStudent(int select, pStudentInfoStruct* ppUser_Info) {
	switch (select) {
	case 1:
		serachStudentInfoByName(ppUser_Info);
		break;
	case 2:
		serachStudentInfoById(ppUser_Info);
		break;
	default:
		break;
	}
